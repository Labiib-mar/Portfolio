...
This programed to record OEU report daily
...
dev_link = r'C:\Users\xxxxxxx\Desktop\teeet\OEU_report.csv'
#prod_link = r'\\dkls16\FOLEE\Spotfire\OEU_Report\OEU_report.csv'
log_dev_link = r'C:\Users\xxxxxxx\Desktop\teeet\log_OEU_report.csv'
#log_prod_link = r'\\dkls16\FOLEE\Spotfire\OEU_Report\log_OEU_report.csv'
if os.path.exists(dev_link):   
    ...
    pastdate= datetime.datetime.strptime('12/31/2018 00:00:00', "%m/%d/%Y %H:%M:%S")

try:

    while(pastdate!=today):
        pastdate = pastdate + datetime.timedelta(1)
        d1 ="\'"+pastdate.strftime('%m/%d/%Y')+"\'"
        connection =cx_Oracle.connect('xxxxxxxxxxxxxxxxxxxx')         #Connection to database please update the password area when necessary
    ...
    logging.basicConfig(filename=log_dev_link,filemode='a',format='%(asctime)s %(message)s',datefmt='%m/%d/%Y %I:%M:%S %p')
